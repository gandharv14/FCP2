...
- **Detection.** Rows whose label contains dividend, distribution, or shareholder payment. Only
  ...
  residual, and an exact two-reference product against a payout row is the payout ratio. Every
  other distribution shape - a `MIN` entitlement cap (0352, 0353), a pass-through copy of a
  floored source row (0256), a share multiplier over floored cash (0350), a lesser-of-two-rows
  clamp under a zero floor (0646) - is rendered mechanically from the parsed formula, so no cap,
  floor, multiplier or reference is dropped. Where the row reads a source that is itself held at
  a `MAX(...,0)` floor, the rendered sentence states that floor. **Every value still needs a
  positive signal and there is no default**: a dividend row whose shape matches nothing here is
  left uncovered. `residual_cash_unfloored` and `capped_at_retained_earnings` are retained for
  vocabulary but are no longer emitted by detection: the unfloored claim proved unprovable from
  the shape alone (0256 and 0350 both floored their cash upstream of the distribution row), and
  the retained-earnings cap sentence omits the cash side of the `MIN` (the 0646 defect).
...
- **Sentence.**
  - `applied` - "The row labelled {label} is scaled by the ownership share in
    {ingredient}."
